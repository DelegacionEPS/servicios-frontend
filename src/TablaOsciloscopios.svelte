...
<Modal
    ...
    <form
        ...
        <p class="dark:text-white recompensa:text-white">
            Vas a reservar el puesto {puesto} el día {selectedDia.getDate()}/{selectedDia.getMonth() +
                1}/{selectedDia.getFullYear()}, de {selectedHora}:00 a {selectedHora + 2}:00. Para
            reservarlo, debes de estar de acuerdo con estas normas:
            <br />
            <br />
            - Como mucho podrán acudir 3 personas al puesto reservado.
            <br />
        ...
        <Label class="space-y-2 recompensa:text-white">
            ...
            <Input
                ...
                value="{selectedDia.getDate()}/{selectedDia.getMonth() +
                    1}/{selectedDia.getFullYear()}"
                required
                readonly />
        </Label>
...
<Modal
    ...
    <p class="dark:text-white recompensa:text-white">
        Debes iniciar sesión para reservar un puesto
    </p>
...
<Modal
    bind:open={formModalDateError}
    size="xs"
    autoclose={false}
    class="w-full recompensa:bg-recompensa-secondary">
    <h3 class="text-xl font-medium text-gray-900 dark:text-white recompensa:text-white">
        Fecha Errónea
    </h3>
...
{#if selectedWeek == week1}
    <h1
        class="sm:text-5xl text-3xl text-center w-full py-6 dark:text-white recompensa:text-white dark:bg-[#070a17] recompensa:bg-recompensa-background">
        Semana Actual
    </h1>
{:else}
    <h1
...
<div class="grid grid-cols-1 place-self-center mt-6">
    <div
        class="w-auto m-auto dark:text-white recompensa:text-white grid sm:grid-cols-3 grid-cols-2">
...
<div class="w-10/12 overflow-auto m-auto mt-6">
    <table style="border: 2px solid black; border-radius: 13px; border-spacing: 0; " class="m-auto">
        <thead>
            <tr>
                        ...
                        {#if plantilla["plantilla"][day.getDay() + "-" + start_hour] != 0}
                            ...
                            {:else}
                                <td
                                    ...
                                    on:click={() => {
                                        asignar_horas_dia(day, start_hour)
                                    }}>
                                    {start_hour}:00 - {start_hour + 2}:00
                                </td>
                            {/if}
...
<div class="grid grid-cols-1 place-items-center mt-6 mb-12">
    <Button
        class="dark:bg-dark-primary dark:hover:bg-dark-accent recompensa:bg-recompensa-primary hover:recompensa:bg-recompensa-accent bg-[#3BC4A0] hover:bg-[#FF6D2E]"
